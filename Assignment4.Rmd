...
Remember to run this code and update the plots!
...
```{r}
meta = readxl::read_excel("D:/assignment_4/Assignment4MetaData.xlsx")
...
pitch= readxl::read_excel("D:/assignment_4/Assignment4PitchDatav2.xlsx")
...
```{r}
#outcome isn't normally distributed 
...
#more chains and more iterations
model_skeptical <- map2stan( model_skep , chains=2 , cores=2, iter = 20000 )

precis(model_skeptical)

#check if the chains are doing alright
plot(model_skeptical)


#plot results
pairs(model_skeptical)

#check quality
#samples = extract.samples( model_skeptical , n=1e4 )

prediction <- sim( model_skeptical , data=new_data, n= 1e4) #, post=samples 
...
#for other two: the meta analytic prior doesn't change them 
model_meta <- map2stan(
  alist(
    sd_scaled ~ dnorm( mu , sigma ) ,
    mu <- a + bd*diagnosis ,
...
#more chains and more iterations
model_meta_analytic <- map2stan( model_meta , chains=2 , cores=2, iter = 20000 )

precis(model_meta_analytic)

#check if the chains are doing alright
plot(model_meta_analytic)


#plot results
pairs(model_meta_analytic)

#check quality
#samples = extract.samples( model_meta_analytic , n=1e4 )

prediction <- sim( model_meta_analytic , data=new_data, n= 1e4) #, post=samples
...
posterior = extract.samples( model_skeptical , n=1e4 )
...
posterior = extract.samples( model_meta_analytic , n=1e4 )
...
#WAIC
 compare(model_skeptical, model_meta_analytic)
